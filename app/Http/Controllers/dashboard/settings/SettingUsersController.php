    ...
    public function generatePDF(Request $request)
{
    ...
    public function userPrivilege(Request $request, $user_id){
        ...
        $authUserPrivilege = Auth::user()->privilege;
        $systemUserPrivilages = SystemUsers::where('privilege', '>', $authUserPrivilege)
             ->orderBy('id')
             ->get();
    ...
    public function userImportPrivilege(Request $request)
    {
        $message = '';
        ...
        if ($request->isMethod('post')) {
            ...
            $request->validate([
                'import_user_id' => 'required', // Ensure the import_user_id exists and is valid
                'to_user_ids' => 'required', // Ensure to_user_ids is provided as a string
            ]);

            $import_user_id = $request->import_user_id;
            $importRoutesPermissions = RoutesPermissions::where('user_id', $import_user_id)->get();

            if (!$importRoutesPermissions) {
                return response()->json(['message' => 'No permissions found for the import user.'], 404);
            }

            $datasetUsers = $request->to_user_ids;
            $recordedUsers = explode(',', $datasetUsers);

            // Delete existing permissions for the target users
            RoutesPermissions::whereIn('user_id', $recordedUsers)->delete();
            //$permissionsTypesPrivilages = PermissionsTypes::where('route', 'route')->get();
            $routesData = [];
            foreach ($importRoutesPermissions as $importRoutesPermission){
                foreach ($recordedUsers as $recordedUser) {
                    $routesData[] = [
                        'user_id' => $recordedUser, // Assign permissions to the recorded user, not the import user
                        'main_route' => $importRoutesPermission->main_route,
                        'route' => $importRoutesPermission->route,
                        'userType' => $importRoutesPermission->userType,
                        'permission_type' => $importRoutesPermission->permission_type,
                    ];
                }
            }
