...
class CollectionBureausController extends Controller
{
    public function index($route = null){
        ...
        $collectionBureausDetails = CollectionBureaus::all();
        ...
        if($countCheckThisRoutes == 0){
            ...
            return view('pages.dashboard.settings.collectionbureaus', compact('mainMenus','subsMenus', 'data','mainRouteName', 'remindersRoute', 'parentid','routesPermissions','getAllRoutePermisssions','routepermissions','collectionBureausDetails'));
        }
    }



    public function addCollectionBureaus(Request $request){
        $messageType = '';
        $message = '';
        ...
        if ($validator->fails()) {
            ...
            $proData = [
                ...
                'address' => $request->cb_address,
                'telephone' => $request->cb_telephone,
                'mobile' => $request->cb_mobile,
                'email' => $request->cb_email,
                'user_id' => 0,
                'created_by' => Auth::user()->id,
            ];

            // Assuming DepartmentHead is the model class for the table
            $addDatas = new CollectionBureaus();
            ...
            $messageType = 'success';
            $message = 'You have successfully Added the Collection Bureaus data to the database..';
        }
        $responseData = [
            'message' => $message,
    ...
    public function updateCollectionBureaus(Request $request,$pro_id){
        $messageType = '';
        $message = '';

        try {
                $edit_id = $request->edit_id;
                $getTaxes = CollectionBureaus::find($request->edit_id);
                ...
                if ($validator->fails()) {
                    ...
                    $messageType = 'success';
                    $message = 'You have successfully Updated the Collection Bureaus data to the database..';
                }

        } catch (\Exception $e) {
            // Catch any exception and return a response
            $messageType = 'error';
            $message = 'An error occurred while updating the Collection Bureaus .'.$e->getMessage();
    ...
    public function editCollectionBureaus(Request $request,$cat_id){
        //ProductCategories MiniPosConfigurations AcAccounts
        $getDatas = CollectionBureaus::find($request->id);
        //$departments = Departments::all();
        //$departmentHeads = DepartmentHeads::all();

        if (!$getDatas) {
            return response()->json(['error' => 'Collection Bureaus are not found'], 404);
        }
        $responseData = [
            'collectionbureaus' => $getDatas
        ];

        return response()->json($responseData);
    }

    public function deleteCollectionBureaus(Request $request,$cat_id){
        $messageType = '';
        $message = '';

        $getDatas = CollectionBureaus::find($request->delete_record_id);
        ...
        if($request->delete_record_type == 'inactive'){
            $actveData = 0;
            $message = 'You have successfully Deactivate this Collection Bureaus record..';
        }else{
            $actveData = 1;
            $message = 'You have successfully Activate this Collection Bureaus record..';
        }
    ...
    public function fetchCollectionBureaus(Request $request){
        $query = CollectionBureaus::query();
        ...
        if ($fetchTableDetails->count() > 0) {
            ...
            foreach ($fetchTableDetails as $key => $fetchDetail) {
                ...
                $deletebtn = '';
                $deleteRoutePath = 'collectionbureaus.deletecollectionbureaus';
                if ($canDelete) {
                ...
                $responses .= '<tr>
                                    ...
                                    <td style="vertical-align: middle;">'.$fetchDetail->address.'</td>
                                    <td style="vertical-align: middle;">'.$fetchDetail->telephone.'</td>
                                    <td style="vertical-align: middle;">'.$fetchDetail->mobile.'</td>
                                    <td style="vertical-align: middle;">'.$fetchDetail->email.'</td>
