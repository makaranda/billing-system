...
class CreditCardTypesController extends Controller
{
    public function index($route = null){
        ...
        if($countCheckThisRoutes == 0){
            ...
            return view('pages.dashboard.settings.creditcardtypes', compact('mainMenus','subsMenus', 'data','mainRouteName', 'remindersRoute', 'parentid','routesPermissions','getAllRoutePermisssions','routepermissions','cardTypesDetails'));
        }
    }
    ...
    public function addCreditCardTypes(Request $request){
        $messageType = '';
        $message = '';
        ...
        if ($validator->fails()) {
            ...
            $messageType = 'success';
            $message = 'You have successfully Added the Card Type data to the database..';
        }
        $responseData = [
            'message' => $message,
    ...
    public function updateCreditCardTypes(Request $request,$pro_id){
        $messageType = '';
        $message = '';

        try {
                $edit_id = $request->edit_id;
                $getTaxes = CardTypes::find($request->edit_id);
                ...
                if ($validator->fails()) {
                    ...
                    $messageType = 'success';
                    $message = 'You have successfully Updated the Card Types data to the database..';
                }

        } catch (\Exception $e) {
            // Catch any exception and return a response
            $messageType = 'error';
            $message = 'An error occurred while updating the Card Types .'.$e->getMessage();
    ...
    public function editCreditCardTypesy(Request $request,$cat_id){
        //ProductCategories MiniPosConfigurations AcAccounts
        $getDatas = CardTypes::find($request->id);
        //$departments = Departments::all();
        //$departmentHeads = DepartmentHeads::all();

        if (!$getDatas) {
            return response()->json(['error' => 'Card Types are not found'], 404);
        }
        $responseData = [
            'cardtypes' => $getDatas
        ];
    ...
    public function deleteCreditCardTypes(Request $request,$cat_id){
        $messageType = '';
        $message = '';

        $getDatas = CardTypes::find($request->delete_record_id);
        ...
        if($request->delete_record_type == 'inactive'){
            $actveData = 0;
            $message = 'You have successfully Deactivate this Card Types record..';
        }else{
            $actveData = 1;
            $message = 'You have successfully Activate this Card Types record..';
        }
    ...
    public function fetchproCreditCardTypes(Request $request){
        $query = CardTypes::query();
