...
<div class="container">
    <div class="page-inner">
      <div class="page-header">
        <h3 class="fw-bold mb-3 text-capitalize">Set Employee OT Factor</h3>
        <ul class="breadcrumbs mb-3">
          ...
          <li class="nav-item text-capitalize">
            <a href="#">Set Employee OT Factor</a>
          </li>
        </ul>
      </div>
        @php
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="d-md-12">
                            ...
                            <div class="row">
                                <div class="col-12">
                                     <h1 class="text-uppercase">Set Employee OT Factor</h1>
                                        ...
                                        <div class="col-md-12">
                                            <div class="card">
                                                <div class="card-header">
                                                    <span class="text-uppercase">Set Employee OT Factor Information</span>
                                                    @if(isset($routepermissions['create']) && $routepermissions['create'] == 1)
                                                        <button type="button" class="btn btn-xs btn-info pull-right ml-1 addOTFactorButton" data-bs-toggle="modal" data-bs-target="#addOTFactorModal" role="button">
                                                            <span class="glyphicon glyphicon-plus"></span>
                                                            Add Set Employee OT Factor
                                                        </button>
                                                    @endif
                                                </div>
...
<!-- Add OTFactor Modal -->
<div class="modal fade" id="addOTFactorModal" role="dialog">
    <div class="modal-dialog">

      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title"><span class="fa fa-plus"></span> SET EMPLOYEE OT FACTOR</h4>
          <button type="button" class="close" data-bs-dismiss="modal">&times;</button>
        </div>
        <form id="frm_add_ot_factor" method="post">
            <input type="hidden" name="form_type" id="form_type" value="setemployeeotfactor.addsetemployeeotfactor">
            <div class="modal-body">
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <label class="control-label" for="ot_factor">OT FACTOR</label>
                        <input type="text" class="form-control" id="ot_factor" name="ot_factor" placeholder="Enter OT factor" required="required" />
                    </div>
...
@push('scripts')
    ...
    $('.addTerritoryButton').on('click',function(){
        $('#frm_add_ot_factor').parsley().reset();
        $('#frm_add_ot_factor')[0].reset();

        $('#edit_id').val('');
        $('#form_type').val('setemployeeotfactor.addsetemployeeotfactor');
    });
    ...
    $('#deleteRecordForm').parsley();
        $('#deleteRecordForm').on('submit', function(event){
            ...
            let updateUrl = '{{ route("setemployeeotfactor.deletesetemployeeotfactor", ":id") }}';
    ...
    $('#frm_add_ot_factor').parsley();
    $('#frm_add_ot_factor').on('submit', function(event){
        event.preventDefault();
        $('#overlay').show();
        var edit_id = ($('#edit_id').val())?$('#edit_id').val():'';
        var form_type = '';

        if($('#form_type').val() == 'setemployeeotfactor.editsetemployeeotfactor'){
            form_type = '{{ route("setemployeeotfactor.updatesetemployeeotfactor", ":id") }}';
            form_type = form_type.replace(':id', edit_id);
        }else{
            form_type = '{{ route("setemployeeotfactor.addsetemployeeotfactor") }}';
        }
    ...
    function editRecord(id){
        ...
        $('#form_type').val('setemployeeotfactor.editsetemployeeotfactor');
        $('#edit_id').val(id);

        let updateUrl = '{{ route("setemployeeotfactor.editsetemployeeotfactor", ":id") }}';
        ...
        $.ajax({
            ...
            success : function(response) {
                ...
                $('#addOTFactorModal').modal('show');
                if(response.employeeotfactor.id != ''){
                    $('#addOTFactorModal #ot_factor').val(response.employeeotfactor.ot_factor);
                }
    ...
    function listTableDatas() {
        ...
        $('#overlay').show();
        $.ajax({
                url : "{{ route('setemployeeotfactor.fetchsetemployeeotfactor') }}",
                cache: false,
